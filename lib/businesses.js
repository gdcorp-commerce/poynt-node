 ...
 * @return {MerchantPaymentAccount} merchantPaymentAccount
 */
module.exports.getMerchantPaymentAccount = function getMerchantPaymentAccount(options, next) {
  ...
  this.request({
    url: '/businesses/' + encodeURIComponent(options.businessId) + '/stores/' + encodeURIComponent(options.storeId) + '/merchant-payment-accounts',
    method: 'GET'
  }, next);
}
