 ...
 * @param {String} region
 */
var getPoyntURL = function getPoyntURL(env, region) {
  if (env === 'dev') {
...
module.exports.patch = function (from, to, options) {
  ...
  Object.keys(from).concat(Object.keys(to)).forEach(function (key) {
    ...
    if (!exists(from[key]) && !exists(to[key])) {
      ...
      ops.push({ op: 'add', path: '/' + key, value: to[key] });

    } else if (to[key].constructor === Object || (to[key].constructor === Array && options.traverseArrays)) {
      ops = ops.concat(module.exports.patch(from[key], to[key], options).map(function (v) {
        v.path = '/' + key + v.path;
        return v;
      }));
      ...
      if (module.exports.patch(from[key], to[key], options).length) {
        ops.push({ op: 'replace', path: '/' + key, value: to[key] });
      }
...
module.exports.nakedRequest = function nakedRequest(options, next) {
  ...
  var data = {
    url                : getPoyntURL(options.env, options.region) + options.url,
