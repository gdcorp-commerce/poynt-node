...
var makeRequest = require("retry-request", {
 ...
 * @param {RetryOptions} options.retry (optional) - Retry options
 */
module.exports.nakedRequest = function nakedRequest(options, next) {
  ...
  var retryOps = {
    shouldRetryFn: function (response) {
      var retryEnabled = !!options.retry;
      return [retryEnabled, response.statusCode >= 500, response.req.method == 'GET'].every(condition => condition);
    },
    noResponseRetries: 0, // by default it always retries
    ...options.retry
  }

  return makeRequest(data, retryOps, function (err, res, body) {
    if (!res) {
      res = {
        statusCode: 500,
      };
