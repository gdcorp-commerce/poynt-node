...
/**
 * Creates a single order at a business.
 * @param {String} options.businessId
 * @param {Order} order
 * @return {Order} order
 */
module.exports.createOrder = function createOrder(options, order, next) {
  var hasErr = helpers.hasKeys(options, ["businessId"]);
  if (hasErr) {
    return next(hasErr);
  }
  ...
  this.request(
    {
      url: "/businesses/" + encodeURIComponent(options.businessId) + "/orders",
      method: "POST",
      body: order,
    },
    function (err, order) {
...
/**
 * Completes an order at a business.
 * @param {String} options.businessId
 * @param {String} options.orderId
 * @return {Order} order
 */
module.exports.completeOrder = function completeOrder(options, next) {
  var hasErr = helpers.hasKeys(options, ["businessId", "orderId"]);
  if (hasErr) {
    return next(hasErr);
  }
