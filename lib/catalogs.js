...
module.exports.updateCatalog = function updateCatalog(options, catalogOrPatch, next) {
  ...
  var f = ff(function () {
    ...
    if (!oldCatalog) { return next(helpers.error('NotFoundError', 'Old catalog not found', 404)); }
...
module.exports.createCategory = function createCategory(options, category, next) {
  ...
  this.request({
    url    : '/businesses/' + encodeURIComponent(options.businessId) + '/catalogs/' + encodeURIComponent(options.catalogId) + '/categories',
    method : 'POST',
    body   : category
  }, next);
};
...
module.exports.lookupCategories = function lookupCategories(options, next) {
  ...
  this.request({
    url    : '/businesses/' + encodeURIComponent(options.businessId) + '/catalogs/' + encodeURIComponent(options.catalogId) + '/categories/lookup?' + qs.stringify(query),
    method : 'GET'
  }, next);
};
...
module.exports.updateCategory = function updateCategory(options, categoryOrPatch, next) {
  ...
  var f = ff(function () {
    ...
    if (!oldCategory) { return next(helpers.error('NotFoundError', 'Old category not found', 404)); }
