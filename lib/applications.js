...
module.exports.createApplication = function createApplication(
  ...
  let data = {
    ...
    application: {
      ...
      applicationLevel: options.applicationLevel || 'BASE'
    }
  };

  if (options.applicationLevel !== 'BASE') {
    data = {
      applicant: {
        ...data.applicant,
...
module.exports.passOTPVerification = function passOTPVerification(
  ...
  let hasErr = helpers.hasKeys(options, ['applicationId', 'code']);
  ...
  this.request(
    {
      url: `/applications/${options.applicationId}/step-ups/otp`,
      app: 'WEB',
      method: 'POST',
      body: {
        code: options.code
