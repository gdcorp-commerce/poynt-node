...
module.exports.getReports = function getReports(options, next) {
  ...
  this.request({
    url: '/api/services/businesses/' + encodeURIComponent(options.businessId) + '/reports?' + qs.stringify(query),
    method: 'GET',
    app: 'WEB'
  }, next);
};
...
module.exports.createReport = function createReport(options, next) {
  ...
  this.request({
    url: '/api/services/businesses/' + encodeURIComponent(options.businessId) + '/reports',
    method: 'POST',
    body: options,
    app: 'WEB'
  }, next);
